...
EchoSonos.prototype.eventHandlers.onLaunch = function(launchRequest, session, response){
  // This is when they launch the skill but don't specify what they want. Prompt
  var output = 'Willkommen bei Sonos controll. ' +
    'Welche funktion möchtest du ausführen? ';

  var reprompt = 'Benötigst du Hilfe dann sage Hilfe nach dem Ton';
...
EchoSonos.prototype.intentHandlers = {
    ...
    ChangeRoomIntent: function (intent, session, response) {
        ...
        if (!options.advancedMode) {
           	response.tell("Dieser Befehl funktioniert nur, wenn der erweiterte Modus aktiviert ist");
        } else {
...
function musicHandler(roomValue, service, cmdpath, name, response) {
    ...
    if (service == 'presets') {
        ...
        var msgStart = (cmdpath.startsWith('/station'))?'Started ':'Eingereiht und gestartet ';
    ...
    httpreq(options, function (error, responseJson) {
        if (!error) {
            ...
            if (responseJson.currentTrack.artist != undefined) {
            	...
            	response.tell("Der aktuelle Künstler wurde nicht gefunden");
	        }
        } else { 
            genericResponse(error, response);
        }
    ...
    actOnCoordinator(options, skillPath, roomValue, function(error, responseBodyJson) {
    	if (error) {
        	genericResponse(error, response);
          	...
          	response.tell('Sirius XM ' + type + ' ' + name + ' gestartet');
    	}
    });
}
...
function pandoraHandler(roomValue, cmdpath, name, response) {
    ...
    actOnCoordinator(options, skillPath, roomValue, function(error, responseBodyJson) {
		if (error) {
      		...
      		if (cmdpath == '/play/') {
         		response.tell('Pandora ' + name + ' gestartet');
      		} else {
        		genericResponse(error, response);
      		}  
      	}
...
function playlistHandler(roomValue, presetValue, skillName, response) {
    ...
    actOnCoordinator(options, '/play', roomValue, function(error, responseBodyJson) {
        genericResponse(error, response, "Eingereiht und gestartet " + presetValue);
    });
}
...
function toggleHandler(roomValue, toggleValue, skillName, response) {
    if (!toggleValue || (toggleValue != 'on' && toggleValue != 'off')) {
        response.tell("Ich muss wissen, ob ich  " + skillName + " ein oder ausschalten soll. Beispiel: Alexa, sag Sonos schalte " + skillName + " ein");
        return;
    }
...
function volumeHandler(roomValue, response, volume) {
    ...
    if (!roomAndGroup.room) {
        response.tell("Bitte geben Sie ein Raumname an. Beispiel: Alexa, sag Sonos reduzier die Lautstärke in der KÜCHE");
        return;
    }
...
function genericResponse(error, response, success) {
    if (!error) {
        ...
        response.tell("Der Lambda Dienst hat einen Fehler festgestellt: " + error.message);
    }
}
